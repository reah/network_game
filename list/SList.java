...
public class SList extends List {
   ...
   *  @param x is the x coordinate of where the item is on the board
   *  @param y is the y coordinate of where the item is on the board
   *  @param next the node following this node.
   **/
  protected SListNode newNode(int item, int x, int y, SListNode next) {
    return new SListNode(item, x, y, this, next);
  }
   ...
   *  @param item is the item to be inserted.
	 *  @param x is the x coordinate of the item on the board
	 *  @param y is the y coordinate of the item on the board
   *
   *  Performance:  runs in O(1) time.
   **/
  public void insertFront(int item, int x, int y) {
    head = newNode(item, x, y, head);
    if (size == 0) {
      tail = head;
    }
    size++;
   ...
   *  Performance:  runs in O(1) time.
   **/
  public void insertBack(int item, int x, int y) {
    if (head == null) {
      head = newNode(item, x, y, null);
      tail = head;
    } else {
      tail.next = newNode(item, x, y, null);
      tail = tail.next;
    }
    size++;
  }
  ...
  public ListNode front() {
    if (head == null) {
      ...
      SListNode node = newNode(0, 0, 0, null);
      node.myList = null;
      return node;
    } else {
      return head;
  ...
  public String toString() {
    ...
    while (current != null) {
      result = result + "(" + (current.x - 1) + ", " + (current.y - 1) + ")\n";
